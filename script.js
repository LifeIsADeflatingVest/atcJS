    ...
    function draw(timestamp) {
        ...
        if (timestamp - lastTime >= interval) {
            ...
            for (let i=0;i<runways.length;i++) {
                ...
                activeRunway = getRandomItem(activeRunways); // this might have different consequences on crossing runways, needs to be checked (TODO)
            ...
            //iaf, faf
            let f = findInObjArr(fixes, "rwyDir", activeRunway.dir);
	...
	if (commandType.toLowerCase() == "s") {
        ...
        if (flight.arrivingStatus !== true) {
            ...
            if (approachForIaf.length != 0) {
                str = flight.callsign + ", cleared for the approach runway " + activeRunway.name;
                col = "darkgreen";
...
function getCrosswindComponent() {
    let diff = Math.abs(theWind - activeRunway.dir);
